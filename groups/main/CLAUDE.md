...
1. *Share progress in the group* via `mcp__nanoclaw__send_message` with a `sender` parameter matching their exact role/character name (e.g., `sender: "Researcher"`). This makes their messages identifiable — on WhatsApp they appear as `[Researcher] message`.
...
5. Follow WhatsApp formatting rules (no markdown).
