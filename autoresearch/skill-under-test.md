...
## ⚠️ CRITICAL: Classification Guardrails

**The most common error is over-classification — assigning supporting/moderate codes with insufficient evidence, which pushes VUS → LP or LP → P.**

### Evidence Code Application Standards

Before applying any code, verify it meets the conservative threshold below. If the evidence is borderline or ambiguous, **do NOT apply the code** — insufficient evidence = VUS, not LP.

| Code | APPLY ONLY IF | Do NOT apply if |
|------|---------------|-----------------|
| **PM1** | Variant is in a SPECIFIC mutational hotspot (≥3 independently published pathogenic variants at the same/adjacent positions in literature) OR in a structurally/functionally critical domain (catalytic site, DNA-binding interface) with published domain-function evidence | The gene merely has functional domains, or the variant is in a "generally important region" without specific hotspot data |
| **PM2** | gnomAD AF < 0.0001 confirmed across ALL major ancestry groups (NFE, AFR, EAS, SAS, AMR, ASJ — at minimum NFE + AFR + EAS) | Only one or two populations checked, or data is unavailable |
| **PM3** | The **trans** variant has 3+ star ClinVar classification as Pathogenic/Likely Pathogenic, or has strong independent published pathogenic evidence; phase confirmed by parental testing or long-read sequencing | Trans variant is itself a VUS, or phase is assumed/inferred |
| **PP2** | Gene has confirmed HIGH missense constraint: gnomAD missense z-score > 3.09 retrieved via `get_gene_constraint` | The gene is associated with disease but constraint data was not checked or z-score ≤ 3.09 |
| **PP3** | ≥3 INDEPENDENT computational tools CONCORDANTLY predict pathogenic AND no tool predicts benign (e.g., REVEL > 0.75 AND CADD > 25 AND AlphaMissense > 0.564 AND SIFT < 0.05) | Only 2 tools agree, tools are discordant, or predictors are correlated (PolyPhen-2 + SIFT = 1 independent assessment) |
| **PP4** | Patient has a HIGHLY SPECIFIC, DISTINCTIVE phenotype matching a single-gene disorder with narrow differential (e.g., specific ECG pattern in channelopathy, specific facial gestalt in RASopathy) documented in the clinical notes | Phenotype is merely "compatible" with the disease spectrum, or the condition has broad clinical variability |

### Compound Heterozygous Variants — Conservative Classification

When interpreting variants in **autosomal recessive compound heterozygous** context:

1. **Each variant is classified independently**. The compound het status affects only PM3, not the overall classification logic.
2. **PM3 applies fully (moderate weight)** only when the *trans* variant is definitively pathogenic (ClinVar ≥3 star P/LP, or published functional evidence). If the trans variant is itself a novel VUS or LP with limited evidence, PM3 does not fully apply.
3. **Do not conflate "clinically affected compound het patient" with Pathogenic classification**. The clinical outcome informs PP4 (phenotype specificity), not automatic upgrading of the pathogenicity tier.
4. **LP is often the correct ceiling** for compound het variants in recessive disease when: the trans variant lacks definitive pathogenicity evidence, functional data is absent (no PS3), and evidence relies primarily on PM codes (PM1, PM2, PM3) plus supporting criteria (PP2, PP3, PP4). The combination of multiple moderate + supporting criteria supports LP, not P, in this setting.

### Pre-Classification Verification Checklist

**Complete this checklist BEFORE assigning the final classification:**

```
□ For each MODERATE code applied (PM1–PM6): cite specific source evidence
  - PM1: List the specific pathogenic variants that define the hotspot/domain
  - PM2: Confirm ALL major gnomAD populations checked (not just NFE)
  - PM3: Confirm trans variant ClinVar star rating and phase confirmation method

□ For each SUPPORTING code applied (PP1–PP5): verify conservative threshold
  - PP2: Record gnomAD missense z-score (must be > 3.09)
  - PP3: List each tool and score individually; count must be ≥3 concordant
  - PP4: Describe the SPECIFIC distinctive feature(s) matching a single disorder

□ Count evidence tiers:
  - n_pvs = count of PVS codes
  - n_ps  = count of PS codes
  - n_pm  = count of PM codes
  - n_pp  = count of PP codes

□ Apply combining rules mechanically (see table above)

□ If classification is LP or P, ask: "Could any applied code be downgraded?"
  - If YES: downgrade and re-apply rules. When in doubt, classify more conservatively.

□ Final: Does the evidence actually support this tier, or is uncertainty high?
  - High uncertainty + moderate/supporting codes only = VUS is appropriate
```

### Worked Example: Avoiding VUS → LP Over-classification

**Scenario**: SCN5A missense variant in a Brugada syndrome patient.
- PM1 attempted: Variant in S4-S5 linker of voltage-sensing domain.
  - **STOP**: Is there a specific mutational hotspot with ≥3 published pathogenic variants at this position? If not, PM1 does NOT apply.
- PM2: AF = 0.00008 in gnomAD NFE. Check AFR, EAS, SAS before assigning.
- PP2: Run `get_gene_constraint` for SCN5A. If missense z-score ≤ 3.09 → PP2 does NOT apply.
- PP4: Brugada syndrome phenotype — is the ECG pattern diagnostic (Type 1 spontaneous), or is it a non-specific presentation? "Possible Brugada" ≠ PP4.
- **Result**: If codes cannot all be confirmed at conservative threshold → fewer codes → may classify as VUS, not LP.

### Worked Example: LP vs Pathogenic in Compound Het

**Scenario**: RYR1 compound het in a malignant hyperthermia susceptibility (MHS) patient.
- Codes confirmed: PM1, PM2, PM3, PP2, PP3, PP4
- Mechanical rule check: 3 moderate (PM1+PM2+PM3) + 3 supporting (PP2+PP3+PP4)
  - This satisfies Pathogenic rule (e): ≥2 moderate + ≥2 supporting
- **BUT**: Apply compound het conservatism:
  - Is PM3 fully supported (trans variant is definitively P, phase confirmed)? If the trans variant is LP or novel → PM3 may not apply at full moderate weight
  - With PM1+PM2 (2 moderate) + PP2+PP3+PP4 (3 supporting): satisfies LP rule (f): 1 moderate + ≥4 supporting? No, only 3 supporting.
  - LP rule (e): 2 moderate + ≥2 supporting → YES → Likely Pathogenic
- **Conclusion**: When PM3 is borderline in compound het, LP rule (e) is the appropriate ceiling: **Likely Pathogenic**.
...
- **PP2**: Missense in a gene with high missense constraint (missense z-score > 3.09) — **PP2 requires this z-score to be confirmed; do not apply without it**
...
NOTE: PolyPhen-2 and SIFT are correlated (both SIFT-based families). Count them as ONE
independent data point, not two. Require ≥3 truly independent tools for PP3/BP4.
```
...
5. Mutational hotspot or functional domain? -> PM1 (ONLY if specific hotspot evidence exists)
6. Functional studies? -> PS3/BS3
7. Low benign missense rate? -> PP2 (ONLY if gnomAD missense z-score > 3.09 confirmed)
8. LoF-only gene? -> BP1
...
- Variant in trans with known pathogenic (recessive): PM3 (moderate) — **only if trans variant is confirmed P/LP with ≥3 star ClinVar evidence or strong independent functional/clinical evidence**
...
- **LP is often the correct ceiling** in compound het settings where PM3 is the primary moderate criterion (see Classification Guardrails above)
...
- [ ] Population frequency queried across gnomAD ancestry groups (BA1/BS1/PM2 assessed) — ALL major populations checked
- [ ] gnomAD missense z-score retrieved via `get_gene_constraint` (required before applying PP2)
- [ ] Computational predictions aggregated from at least 3 INDEPENDENT tools (REVEL, CADD, AlphaMissense are preferred; PolyPhen-2 + SIFT count as one independent unit)
...
- [ ] Each ACMG evidence code verified against conservative application standards (see Classification Guardrails)
- [ ] All applicable ACMG evidence codes assigned with T-grade and justification
- [ ] ACMG combining rules applied mechanically and final classification determined (Pathogenic/LP/VUS/LB/Benign)
- [ ] If compound het: PM3 conservatism check applied; LP considered as ceiling unless strong independent evidence
